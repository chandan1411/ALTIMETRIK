package com.ckt.empService.config;
...
@Configuration
@EnableSwagger2
@ComponentScan(basePackages = "com.ckt.empService")
public class SwaggerConfig {

	private ApiInfo metadata() {
		Contact contact = new Contact("EMS Team", "", "emssupport@verizon.com");
		return new ApiInfoBuilder().title("EmployeeProfile Service").description("Employee Profile Management API").version("1.0")
	...
	public Docket documentation() {
		return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.any())
				.paths(PathSelectors.regex("/employee-service.*")).build().apiInfo(metadata());
	}
}